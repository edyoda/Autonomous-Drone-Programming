...
class OffboardPositionControl(Node):
    def __init__(self) -> None:
        ...
        self.target_x = 10.0
        self.target_y = 0.0
        self.target_z = -5.0  # negative = up in many PX4 coordinate conventions
        self.target_yaw = 0.0
    ...
    def timer_callback(self) -> None:
        ...
        # Optional: log current height and nav state every few loops
        if self.setpoint_counter % 20 == 0:
            try:
                current_z = float(self.local_pos.z)
            except Exception:
                current_z = float("nan")
            nav_state = getattr(self.status, "nav_state", None)
            self.get_logger().info(f"local_z={current_z:.2f}, nav_state={nav_state}")
